...
// Beta-gated runtimes (Artemis, Minori) ship compiled into product builds
// but stay locked until a fresh coffee entitlement re-enables the
...
bool ProviderRequiresBetaAccess(DispatchHandle* handle) {
  ...
  return runtime_id == "artemis" || runtime_id == "minori";
}
...
engine_result_t CheckArtemisBetaAccess(DispatchHandle* handle) {
  ...
  const std::string runtime_id = Normalize(handle->provider->runtime_id_utf8);
  const std::string display_name = runtime_id == "minori" ? "Minori" : "Artemis";

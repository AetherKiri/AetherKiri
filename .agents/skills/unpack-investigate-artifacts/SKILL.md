...
description: Safely unpack and investigate existing archives through evidence-first static analysis, especially ZIP bundles already generated by AetherKiri tools/diagnose.py. Use when Codex is given a diagnostic ZIP or directory, TAR, APK, IPA, .app bundle, crash archive, or opaque packaged artifact and needs to validate the bundle, align logs and markers, compare layers, or identify the first unsupported boundary without building, installing, launching, reproducing, or collecting from a device.
---

# Unpack and Investigate Artifacts

Begin with the artifact the user supplied. Do not build, install, launch, reproduce, or run project-specific collection tools unless the user separately requests those actions.
...
An absent UI marker is not automatically a failed bundle. Check for a host marker with `ui_marker_missing=true`, then state which app-side boundary is unavailable.
...
1. Translate the reported symptom into one or more falsifiable boundaries.
2. Anchor the timeline on explicit markers, crash timestamps, sequence numbers, or monotonic clocks. State when clocks cannot be aligned.

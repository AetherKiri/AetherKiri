...
void TVPUninitScriptEngine() {
    ...
    */
    ...
    // Extension class descriptors are process-static, while the registered
    // class objects are owned by the just-released VM.  Allow the replacement
    // VM created by TVPRestartScriptEngine() to install fresh instances.
    TVPResetAtInstallExtensionClass();
}
//---------------------------------------------------------------------------
...
#endif
            // A failed startup script may have already defined classes,
            // singleton objects, or factory functions before throwing.  The
            // legacy fallback is a second complete initialization pass and
            // must not inherit that partially constructed TJS global state;
            // otherwise titles such as DRACU-RIOT see a false
            // createDrawDevice collision on the retry.  Restart only the
            // script engine here: storage mounts, native windows, and the
            // host application remain alive while all script globals and
            // extension registrations are rebuilt cleanly.
            spdlog::info(
                "Restarting TJS engine before fallback system/Initialize.tjs");
            TVPRestartScriptEngine();
            // Initialize.tjs is normally reached through startup.tjs, which
            // links the proxy filesystem first.  The direct fallback must
            // restore that optional module explicitly; otherwise archive
            // paths such as k2compat.xp3 are parsed as an unsupported
            // `proxy:` media type on the second pass.
            TVPLoadInternalPlugins();
            TVPLoadPlugin(TJS_W("proxyfs.dll"));

...
func _show_import_guide() -> void:
    ...
    var box := _modal_stack(dialog, _t("dialog.import_title"), ICON_LIBRARY)
    var body := Label.new()
...
func _show_web_import_picker() -> void:
    ...
    if bool(support.get("archive", false)):
        ...
        archive_button.pressed.connect(func():
            _dismiss_modal(func(): _pick_web_local_game("archive"))
        )
...
func _game_card(game: Dictionary) -> Button:
    ...
    button.pressed.connect(func(): _show_detail(game, button))
...
func _show_android_storage_permission_prompt() -> void:
    ...
    ok.pressed.connect(func():
        _dismiss_modal(func(): call_deferred("_request_android_storage_permissions"))
    )
    box.add_child(ok)

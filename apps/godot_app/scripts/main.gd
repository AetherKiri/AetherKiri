...
func _quit_after_runtime_exit() -> void:
    ...
    _set_game_text_input_active(false)
...
func _set_game_text_input_active(active: bool) -> void:
    if game_text_input_active == active:
        return
    game_text_input_active = active
    var window := get_window()
    if window == null:
        return
    window.set_ime_active(active)
    if active:
        window.set_ime_position(Vector2i(
            int(round(size.x * 0.5)),
            int(round(size.y * 0.5))
        ))
...
func _sync_game_text_input_state() -> void:
    if modal_layer != null and bool(
        ...
        return
    var active: bool = _can_forward_game_input() and player != null and (
        player.has_method("is_text_input_active") and
        bool(player.is_text_input_active())
    )
    _set_game_text_input_active(active)
